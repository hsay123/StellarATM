...
export default function ReceiptScreen({ receipt, onDone }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => setShowReceipt(true), 300)
  ...
  const slotEl = typeof document !== 'undefined'
    ? document.getElementById('receipt-print-zone')
    : null

  const receiptPaper = showReceipt ? (
    <div className="receipt-print-clip">
      <div className="receipt-paper receipt-torn-bottom receipt-curl p-4 sm:p-6 w-full max-w-xs mx-auto relative">
  ...
  return (
    <div className="h-full flex flex-col p-4 sm:p-6 font-terminal text-base sm:text-lg">
      ...
      {/* Portal receipt paper to the printer slot area */}
      {slotEl && createPortal(receiptPaper, slotEl)}
    </div>
  )
}
